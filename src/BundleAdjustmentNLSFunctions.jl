# BundleAdjustmentNLSFunctions.jl
...
# ########################################################################
# # RESIDUAL CALCULATIONS (CPU and GPU)
# ########################################################################

function NLPModels.residual!(nls::BundleAdjustmentModel, x::AbstractVector, rx::AbstractVector)
  increment!(nls, :neval_residual)
  # This now dispatches to either the CPU or GPU version based on the types
...
@inline @inbounds function gpu_projection(p3_1, p3_2, p3_3, c1, c2, c3, c4, c5, c6, c7, c8, c9)
    ...
    k1, k2, k3 = if θ < 1e-9; (0.0, 0.0, 0.0) else (c1 / θ, c2 / θ, c3 / θ) end
    ...
    # P1 = p3*cos(θ) + cross(k,p3)*sin(θ) + k*dot(k,p3)*(1-cos(θ)) + t
...
function residuals!(
  xs::CuVector, rxs::CuVector, cam_indices::CuVector{Int}, pnt_indices::CuVector{Int}, nobs::Int, npts::Int, pt2d::CuVector,
)
...
function residuals!(
  xs::Vector, rxs::Vector, cam_indices::Vector{Int}, pnt_indices::Vector{Int}, nobs::Int, npts::Int, pt2d::Vector,
)
  @simd for i = 1:nobs
    cam_index = cam_indices[i]
    pnt_index = pnt_indices[i]
...
function projection!(p3::AbstractVector, r::AbstractVector, t::AbstractVector, k_1, k_2, f, r2::AbstractVector)
  θ = norm(r)
  k1, k2, k3 = if θ < 1e-9; (0.0, 0.0, 0.0) else (r[1]/θ, r[2]/θ, r[3]/θ) end
...
function cross!(c::AbstractVector, a::AbstractVector, b::AbstractVector)
  ...
  c[1] = a2 * b3 - a3 * b2; c[2] = a3 * b1 - a1 * b3; c[3] = a1 * b2 - a2 * b1
  c
end
...
# ########################################################################
# # JACOBIAN CALCULATIONS (CPU and GPU)
# ########################################################################

function NLPModels.jac_structure_residual!(
  nls::BundleAdjustmentModel, rows::AbstractVector{<:Integer}, cols::AbstractVector{<:Integer},
)
...
@inline @inbounds function gpu_jacobian_block(X_x, X_y, X_z, C_rx, C_ry, C_rz, C_tx, C_ty, C_tz, C_k1, C_k2, C_f)
    T = typeof(X_x)
    θ = sqrt(C_rx^2 + C_ry^2 + C_rz^2)
    k_x, k_y, k_z = if θ < 1e-9; (T(0.0), T(0.0), T(0.0)) else (C_rx / θ, C_ry / θ, C_rz / θ) end
    cos_θ, sin_θ = cos(θ), sin(θ)

    R11 = cos_θ + (1 - cos_θ) * k_x^2; R12 = (1 - cos_θ) * k_x * k_y - sin_θ * k_z; R13 = (1 - cos_θ) * k_x * k_z + sin_θ * k_y
    R21 = (1 - cos_θ) * k_y * k_x + sin_θ * k_z; R22 = cos_θ + (1 - cos_θ) * k_y^2; R23 = (1 - cos_θ) * k_y * k_z - sin_θ * k_x
    R31 = (1 - cos_θ) * k_z * k_x - sin_θ * k_y; R32 = (1 - cos_θ) * k_z * k_y + sin_θ * k_x; R33 = cos_θ + (1 - cos_θ) * k_z^2
    ...
    JP2_11 = -1.0 / P1_z; JP2_13 = P1_x * inv_P1_z_sq
    JP2_22 = -1.0 / P1_z; JP2_23 = P1_y * inv_P1_z_sq
    P2_x = -P1_x / P1_z; P2_y = -P1_y / P1_z

    norm2 = P2_x^2 + P2_y^2; norm4 = norm2^2
    r_dist = 1 + C_k1 * norm2 + C_k2 * norm4
    dr_dx = 2 * C_k1 * P2_x + 4 * C_k2 * norm2 * P2_x; dr_dy = 2 * C_k1 * P2_y + 4 * C_k2 * norm2 * P2_y
    JP3_11 = C_f * (r_dist + P2_x * dr_dx); JP3_12 = C_f * (P2_x * dr_dy)
    JP3_21 = C_f * (P2_y * dr_dx); JP3_22 = C_f * (r_dist + P2_y * dr_dy)
    ...
    JM_11 = JP3_11 * JP2_11; JM_12 = JP3_12 * JP2_22; JM_13 = JP3_11 * JP2_13 + JP3_12 * JP2_23
    JM_21 = JP3_21 * JP2_11; JM_22 = JP3_22 * JP2_22; JM_23 = JP3_21 * JP2_13 + JP3_22 * JP2_23

    J_x1 = JM_11*R11 + JM_12*R21 + JM_13*R31; J_x2 = JM_11*R12 + JM_12*R22 + JM_13*R32; J_x3 = JM_11*R13 + JM_12*R23 + JM_13*R33
    J_y1 = JM_21*R11 + JM_22*R21 + JM_23*R31; J_y2 = JM_21*R12 + JM_22*R22 + JM_23*R32; J_y3 = JM_21*R13 + JM_22*R23 + JM_23*R33
    ...
    term1 = -sin_θ * d + (1 - cos_θ) * inv_θ * d; term2 = sin_θ * inv_θ
    ...
    J_r1_1 = JM_11*dr_11 + JM_12*dr_21 + JM_13*dr_31; J_r1_2 = JM_11*dr_12 + JM_12*dr_22 + JM_13*dr_32; J_r1_3 = JM_11*dr_13 + JM_12*dr_23 + JM_13*dr_33
    J_r2_1 = JM_21*dr_11 + JM_22*dr_21 + JM_23*dr_31; J_r2_2 = JM_21*dr_12 + JM_22*dr_22 + JM_23*dr_32; J_r2_3 = JM_21*dr_13 + JM_22*dr_23 + JM_23*dr_33

    return (
        J_x1, J_y1, J_x2, J_y2, J_x3, J_y3,                         # Cols 1-3 (∂P/∂X)
        J_r1_1, J_r2_1, J_r1_2, J_r2_2, J_r1_3, J_r2_3,             # Cols 4-6 (∂P/∂r)
...
function NLPModels.jac_coord_residual!(
  nls::BundleAdjustmentModel, x::CuVector, vals::CuVector,
)
...
function NLPModels.jac_coord_residual!(
  nls::BundleAdjustmentModel, x::Vector, vals::Vector,
)
  increment!(nls, :neval_jac_residual)
  T = eltype(x)
  ...
  @simd for i = 1:(nls.nobs)
    ...
    @views X = x[((idx_pnt - 1) * 3 + 1):((idx_pnt - 1) * 3 + 3)]
    @views C = x[(3 * nls.npnts + (idx_cam - 1) * 9 + 1):(3 * nls.npnts + (idx_cam - 1) * 9 + 9)]
    @views r = C[1:3]
    @views t = C[4:6]
